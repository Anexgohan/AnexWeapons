...
//NEW NEW VERSION: replaced MesoTroniK's old muzzle-flash system with Nicke535's newer, modular (and horribly long) version
...
public class anexweapons_ltb_parashu_turret_everyFrame implements EveryFrameWeaponEffectPlugin {
	...
	static {
		HARDPOINT_OFFSETS.put(0, new Vector2f(5f, -10f));
		HARDPOINT_OFFSETS.put(1, new Vector2f(5f, 10f));
	}
	private static final Map<Integer, Vector2f> TURRET_OFFSETS = new HashMap<Integer, Vector2f>();
	static {
		TURRET_OFFSETS.put(0, new Vector2f(-9f, -10f));
		TURRET_OFFSETS.put(1, new Vector2f(-9f, 10f));
	}
    ...
    /*
        ...
        HOW TO USE:
        USED_IDS specifies which IDs to use for the rest of the script; any ID is valid EXCEPT the unique ID "default".
        Each ID should only be used once on the same weapon. The script will spawn one particle "system" for each ID
        in this list, with the specific attributes of that ID.

        All the different Maps<> specify the attributes of each of the particle "systems"; they MUST have something
        defined as "default", and can have specific fields for specific IDs in the USED_IDS list; any field not filled
        in for a specific ID will revert to "default" instead.

    */
	private static final List<String> USED_IDS = new ArrayList<>();
	static {
		USED_IDS.add("POINT_GLOW");
		USED_IDS.add("SPRAY");
	}
	...
	static {
		ON_SHOT_PARTICLE_COUNT.put("default", 50);
		ON_SHOT_PARTICLE_COUNT.put("SPRAY", 150);
	}
	...
	static {
		PARTICLES_PER_SECOND.put("default", 5f);
	}
	...
	static {
		...
		PARTICLE_SPAWN_MOMENT.put("SPRAY", "FIRING");
	}
	...
	static {
		...
		PARTICLE_SPAWN_POINT_TURRET.put("SPRAY", new Vector2f(0f, 0f));
	}
	...
	static {
		...
		PARTICLE_SPAWN_POINT_HARDPOINT.put("SPRAY", new Vector2f(0f, 0f));
	}
	...
	static {
		...
		PARTICLE_SIZE_MAX.put("SPRAY", 2f);
	}
	...
	static {
		...
		PARTICLE_VELOCITY_MIN.put("SPRAY", 0f);
	}
	...
	static {
		PARTICLE_VELOCITY_MAX.put("default", 25f);
		PARTICLE_VELOCITY_MAX.put("SPRAY", 95f);
	}
	...
	static {
		...
		PARTICLE_DURATION_MIN.put("SPRAY", 0.4f);
	}
	...
	static {
		...
		PARTICLE_DURATION_MAX.put("SPRAY", 0.95f);
	}
	...
	/*-----           -----SCRIPT-USED VARIABLES:DON'T TOUCH-----           -----*/
	//These ones are for the alternator part
	private int alternatorCounter = 0;
	private boolean alternatorRunOnce = true;

	//These ones are for the muzzle flash part
	...
	public void advance(float amount, CombatEngineAPI engine, WeaponAPI weapon) {
		...
		//Runs the muzzleFlash part of the code
		muzzleFlashAdvance(amount, engine, weapon);

		//When we are firing, reset our runOnce variable so we know we have started firing again
		float chargeLevel = weapon.getChargeLevel();
		if (chargeLevel > 0) {
			alternatorRunOnce = true;
		}

		//Once we stop firing, switch our beams one step along our list of offsets
		if (weapon.getChargeLevel() <= 0f && alternatorRunOnce) {
			alternatorRunOnce = false;
	...
	//The advance function for the muzzle flash effects. Separated to avoid some clutter
	private void muzzleFlashAdvance(float amount, CombatEngineAPI engine, WeaponAPI weapon) {
