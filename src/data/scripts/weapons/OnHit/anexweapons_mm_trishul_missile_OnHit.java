...
//copied from tahlan_PhiraOnHitEffect by Nia Tahl

public class anexweapons_mm_trishul_missile_OnHit implements OnHitEffectPlugin {

    private static final Color COLOR1 = new Color(255,130,30,255);
    private static final Color COLOR2 = new Color(255, 246, 234,255);
    private static final float EMP_MULT = 0.5f;
    private static final float EMP_ARC_RADIUS = 1000f;
    private static final float DAMAGE_MULT = 0.05f;
    ...
    public void onHit(DamagingProjectileAPI projectile, CombatEntityAPI target, Vector2f point, boolean shieldHit, ApplyDamageResultAPI damageResult, CombatEngineAPI engine) {
        ...
        SpriteAPI spriteToUse = Global.getSettings().getSprite("fx","anexweapons_trail_fuzzy");
        for (int i1 = 0; i1 < 5; i1++) {
            ...
            float lifetimeMult = MathUtils.getRandomNumberInRange(0.3f, 1.5f);
        ...
        if (target instanceof ShipAPI) {
            ...
            float hitLevel = 0f;
            float emp = projectile.getEmpAmount() * EMP_MULT;
            float dam = projectile.getDamageAmount() * DAMAGE_MULT;
            for (int x = 0; x < 4; x++) {
                float pierceChance = 0.25f;
                boolean triggered = (float) Math.random() < pierceChance;
                //(!shieldHit && triggered) modified below to create lightning even on shield hit
                if (triggered) {
                    hitLevel += 0.25f;
                    ShipAPI empTarget = ship;
                    EmpArcEntityAPI arc =  engine.spawnEmpArcPierceShields(projectile.getSource(), point, empTarget, empTarget,
                            projectile.getDamageType(), dam, emp, EMP_ARC_RADIUS * hitLevel, null, 20f, COLOR1, COLOR2);
                }
            }
